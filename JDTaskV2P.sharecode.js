...
// V2PKey: JdPetV2P
const PlantBeanCodeList = new Array();
// V2PKey: PlantBeanV2P
const JDFruitCodeList = new Array();
// V2PKey: JDFruitV2P
const JDCashCodeList = new Array();
// V2PKey: JDCashV2P
const JDSGMHCodeList = new Array();
// V2PKey: JDSGMHV2P
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
                    ...
                    const jdfcCode = item.assistTaskDetailVo.taskToken;
                    JdFactoryCodeList.push(jdfcCode)
                    const jdfcCodeList = JdFactoryCodeList.join("&");
                    $store.put(jdfcCodeList, 'JdFactoryV2P', 'string') ? console.log(`【京东账号${$.index}（${$.UserName}）】JdFactoryV2P保存成功！🎉`) : console.log(`【京东账号${$.index}（${$.UserName}）】JdFactoryV2P失败！❌`);
                  }
                });
              }
            }
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  const shareCode = data.user.encryptPin;
                  JxFactoryCodeList.push(shareCode)
                  const sharecodeList = JxFactoryCodeList.join("&");
                  $store.put(sharecodeList, 'JxFactoryV2P', 'string') ? console.log(`【京东账号${$.index}（${$.UserName}）】JxFactoryV2P保存成功！🎉`) : console.log(`【京东账号${$.index}（${$.UserName}）】JxFactoryV2P失败！❌`);
      ...
      try {
        if (err) {
          ...
          if (
            ...
            const shareCode = $.petInfo.shareCode;
            JdPetCodeList.push(shareCode)
            const sharecodeList = JdPetCodeList.join("&");
            $store.put(sharecodeList, 'JdPetV2P', 'string') ? console.log(`【京东账号${$.index}（${$.UserName}）】JdPetV2P保存成功！🎉`) : console.log(`【京东账号${$.index}（${$.UserName}）】JdPetV2P失败！❌`);
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
      ...
      const shareCode = $.myPlantUuid;
      PlantBeanCodeList.push(shareCode)
      const sharecodeList = PlantBeanCodeList.join("&");
      $store.put(sharecodeList, 'PlantBeanV2P', 'string') ? console.log(`【京东账号${$.index}（${$.UserName}）】PlantBeanV2P保存成功！🎉`) : console.log(`【京东账号${$.index}（${$.UserName}）】PlantBeanV2P失败！❌`);
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      const shareCode = $.farmInfo.farmUserPro.shareCode;
      JDFruitCodeList.push(shareCode)
      const sharecodeList = JDFruitCodeList.join("&");
      $store.put(sharecodeList, 'JDFruitV2P', 'string') ? console.log(`【京东账号${$.index}（${$.UserName}）】JDFruitV2P保存成功！🎉`) : console.log(`【京东账号${$.index}（${$.UserName}）】JDFruitV2P失败！❌`);
    } else {
      /*console.log(
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.data.bizCode === 0) {
            ...
            const shareCode = invites && invites[0]['assistTaskDetailVo']['taskToken'];
            JDSGMHCodeList.push(shareCode)
            const sharecodeList = JDSGMHCodeList.join("&");
            $store.put(sharecodeList, 'JDSGMHV2P', 'string') ? console.log(`【京东账号${$.index}（${$.UserName}）】JDSGMHV2P保存成功！🎉`) : console.log(`【京东账号${$.index}（${$.UserName}）】JDSGMHV2P失败！❌`);
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.code===0 && data.data.result){
              ...
              const shareCode = data.data.result.inviteCode;
              JDCashCodeList.push(shareCode)
              const sharecodeList = JDCashCodeList.join("&");
              $store.put(sharecodeList, 'JDCashV2P', 'string') ? console.log(`【京东账号${$.index}（${$.UserName}）】JDCashV2P保存成功！🎉`) : console.log(`【京东账号${$.index}（${$.UserName}）】JDCashV2P失败！❌`);
            }
          }
        }
      } catch (e) {
