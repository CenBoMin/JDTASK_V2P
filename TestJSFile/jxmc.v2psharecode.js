...
async function getJXMC() {
    try {
      ...
      if (JSON.stringify($.homeInfo) === '{}') {
        ...
        const shareCode = $.homeInfo.sharekey;
        JXMCCodeList.push(shareCode)
        const sharecodeList = JXMCCodeList.join("&");
        $store.put(sharecodeList, 'JXMCV2P', 'string') ? console.log(`【京东账号${$.index}（${$.UserName}）】JXMCV2P保存成功！🎉`) : console.log(`【京东账号${$.index}（${$.UserName}）】JXMCV2P失败！❌`);
      }
    } catch (e) {
      $.logErr(e)
    }
