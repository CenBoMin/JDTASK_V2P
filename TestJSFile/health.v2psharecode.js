...
const JDHealthCodeList = new Array();
// V2PKey: JDHealthV2P
const inviteCodes = []
let reward = ''
          ...
          if (safeGet(data)) {
            ...
            if (taskId === -1) {
              ...
              if (data?.data?.result?.taskVos) {
                ...
                const shareCode = data?.data?.result?.taskVos[0].assistTaskDetailVo.taskToken;
                JDHealthCodeList.push(shareCode)
                const sharecodeList = JDHealthCodeList.join("&");
                $store.put(sharecodeList, 'JDHealthV2P', 'string') ? console.log(`【京东账号${$.index}（${$.UserName}）】JDHealthV2P保存成功！🎉`) : console.log(`【京东账号${$.index}（${$.UserName}）】JDHealthV2P失败！❌`);
