...
!(async () => {
  ...
  if (allAc.length > 0) {
    await Promise.all(allAc.map((ac, i) => carReward(ac)))
    }
})()
  .catch((e) => {
    $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
...
//查询
function getPoint(ac) {
    return new Promise(resolve => {
      $.get(taskUrl(ac,'v1/user/point'), async (err, resp, data) => {
        try {
          if (err) {
...
//换京豆
// function exchange(ac) {
//     return new Promise(resolve => {
//       $.post(taskUrl(ac,'v1/user/exchange/bean/check'), (err, resp, data) => {
//         try {
//           if (err) {
...
//               message += `兑换结果：${JSON.stringify(data)}\n`
//             }
//           }
//         } catch (e) {
//           $.logErr(e, resp)
...
function exchange(ac) {
    return new Promise(resolve => {
      $.get(taskUrl(ac,'v1/user/exchange/bean/check'), (err, resp, data) => {
        try {
          if (err) {
            data = JSON.parse(resp.body)
            console.log(`${data.error.msg}`)
...
// function taskUrl2(ac,function_id, body = {}) {
//   return {
...
function taskPostUrl(ac,function_id, body = {}) {
  return {
    url: `${JD_API_HOST}${function_id}?timestamp=${new Date().getTime() + new Date().getTimezoneOffset() * 60 * 1000 + 8 * 60 * 60 * 1000}`,
